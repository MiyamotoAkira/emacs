...
The below is a wrapper that allows me to add the display-action to lsp-find-definition.
This is because sometimes I want to open the definition in a separate buffer.
If we wanted to removed we need to call (advice-remove 'lsp-find-definition #'jgg/lsp-find-definition-other)
#+BEGIN_SRC emacs-lisp
(defun jgg/lsp-find-definition-other (original-function &optional arg)
  (interactive "P")
  (if arg
      (funcall original-function :display-action 'window)
      (funcall original-function)))

(advice-add 'lsp-find-definition :around #'jgg/lsp-find-definition-other)
#+END_SRC
...
If we wanted to removed we need to call (advice-remove 'lsp-find-definition #'jgg/lsp-find-definition-other)
#+BEGIN_SRC emacs-lisp
(defun jgg/lsp-find-in-other-buffer (original-function &optional arg)
  (interactive "P")
  (if arg
      (funcall original-function :display-action 'window)
      (funcall original-function)))
